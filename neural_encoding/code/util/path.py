...
class Path(object):
    ...
    def dirname(self) -> str:
        ...
        if self.root is not None:
            ...
            if "sub" in self.entities:  # Add derivatives if it's a subject path
                dirnames.append("derivatives")
    ...
    def match(self) -> Iterable[str]:
        return glob(self.fpath)
    ...
    @staticmethod
    def frompath(filename: str, castint=False):
        ...
        if len(dir_parts) == 3:
            root = dir_parts[0]
            datatype = dir_parts[2]
        # else:
        #     print("WARN: unknown number of parts", len(dir_parts))

        return Path(**parts, root=root, datatype=datatype, suffix=suffix, ext=ext)
