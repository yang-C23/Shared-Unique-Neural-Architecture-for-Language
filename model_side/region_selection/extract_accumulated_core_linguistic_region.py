...
# 模型名称和路径
model_name = "mistral-12b"
original_model_path = 'scratch_link/mistral_12b/models/Mistral-Nemo-Base'  # 原版llama模型
...
parser = argparse.ArgumentParser(description='Extract accumulated core linguistic regions')
parser.add_argument('--k', type=float, default=0.01, help='Ratio for top/bottom parameters selection (default: 0.15)')
    ...
    with tqdm(total=400) as pbar:
        ...
        for (name, params) in original_model.named_parameters():
            ...
            for language in language_list:
                ...
                file_dir = 'scratch_link/mistral_12b/path_to_save_step_{}/grad-mul-param_checkpoint_{}'.format(language,samples)
                save_path = os.path.join(file_dir,'{}.pt'.format(name.replace('module.','')))
                #grad_tensor += torch.load(save_path).abs().cpu()
                grad_tensor += torch.load(save_path, map_location=torch.device('cpu')).abs().cpu()
                #torch.load(save_path, map_location='cpu').abs().cpu()

            print(grad_tensor.shape)
            print(grad_tensor.dtype)
            print(grad_tensor.device)
            print(grad_tensor.requires_grad)
            print(grad_tensor.grad)
            print(grad_tensor.grad_fn)
        ...
        # 将输出结果保存到CSV文件
        os.makedirs('scratch_link/mistral_12b/path_to_save_region_selection',exist_ok=True)

        output_file = "scratch_link/mistral_12b/path_to_save_region_selection/three-countries-accumulated-grad-mul-param_top-bottom-{}-{}-{}.csv".format(model_name,k,samples)
        ...
        os.makedirs('scratch_link/mistral_12b/path_to_save_region_selection/three-countries-accumulated-{}-grad-mul-param/{}/top{}'.format(model_name,samples,k),exist_ok=True)
        for key,values in top_k_params_dict.items():
            # 将布尔矩阵转换为字节类型的张量
            save_path = os.path.join('scratch_link/mistral_12b/path_to_save_region_selection/three-countries-accumulated-{}-grad-mul-param/{}/top{}'.format(model_name,samples,k),"{}.pt".format(key))
            # 使用torch.save()保存张量到文件
            torch.save(values, save_path)

        os.makedirs('scratch_link/mistral_12b/path_to_save_region_selection/three-countries-accumulated-{}-grad-mul-param/{}/bottom{}'.format(model_name,samples,k),exist_ok=True)
        for key,values in bottom_k_params_dict.items():
            save_path = os.path.join('scratch_link/mistral_12b/path_to_save_region_selection/three-countries-accumulated-{}-grad-mul-param/{}/bottom{}'.format(model_name,samples,k),"{}.pt".format(key))
            torch.save(values, save_path)

        if samples == 100000: # 如果已经保存过random k 就不用再筛选
            continue

        os.makedirs('scratch_link/mistral_12b/path_to_save_region_selection/three-countries-accumulated-{}-grad-mul-param/{}/random{}'.format(model_name,samples,k),exist_ok=True)
        for key,values in random_k_params_dict.items():
            save_path = os.path.join('scratch_link/mistral_12b/path_to_save_region_selection/three-countries-accumulated-{}-grad-mul-param/{}/random{}'.format(model_name,samples,k),"{}.pt".format(key))
            torch.save(values, save_path)
