...
"""
由于qwen的词表大小约为152064，因此存储的时候使用np.uint32存储，一个数字占4个字节，范围在0~2^32-1
    每个sample的长度不固定
    索引需要记录每个句子的起始位置和长度
...
class Tokenizer:
    def __init__(self, model_path: str):
        ...
        # BOS / EOS token IDs for Qwen
        self.n_words: int = self.sp_model.vocab_size
        self.eos_id: int = self.sp_model.eos_token_id
        self.pad_id: int = self.sp_model.pad_token_id if self.sp_model.pad_token_id is not None else self.sp_model.eos_token_id
        # Qwen models typically don't use BOS tokens during training
        self.bos_id: int = self.sp_model.bos_token_id if hasattr(self.sp_model, 'bos_token_id') and self.sp_model.bos_token_id is not None else None
...
def get_args():
    ...
    parser.add_argument("--tokenizer_path", default='Qwen/Qwen2.5-7B',type=str, help="Tokenizer文件")
...
def write(args):
    ...
    """开始写入"""
    """起始位置:np.uint64: 8B"""
    """长度:np.uint32: 4B (updated for Qwen)"""
    """token:np.uint32: 4B (updated for Qwen)"""
    ...
    for doc in encoded_samples:
        ...
        for i in idx:
            ...
            # Updated token statistics for Qwen
            tokenizer = Tokenizer(args.tokenizer_path)
            if len(target) > 0:
                if target[0] == tokenizer.bos_id and target[-1] == tokenizer.eos_id:
                    statistic[3] += 1
                elif target[0] == tokenizer.bos_id and target[-1] != tokenizer.eos_id:
                    statistic[0] += 1
                elif target[0] != tokenizer.bos_id and target[-1] == tokenizer.eos_id:
                    statistic[1] += 1
                else:
