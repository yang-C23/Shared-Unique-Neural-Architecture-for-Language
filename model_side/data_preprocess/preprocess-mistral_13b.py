...
"""
由于mistralai/Mistral-Nemo-Base-2407的词表大小为131072（~128k），因此存储的时候使用np.uint32存储，一个数字占4个字节，范围在0~2^32-1
    每个sample的长度不固定
    索引需要记录每个句子的起始位置和长度
...
"""mistralai/Mistral-Nemo-Base-2407的分词器"""
class Tokenizer:
    def __init__(self, model_path: str):
        # reload tokenizer
        self.sp_model = AutoTokenizer.from_pretrained(model_path)
        
        # BOS / EOS token IDs for mistralai/Mistral-Nemo-Base-2407
...
def get_args():
    ...
    parser.add_argument("--tokenizer_path", default='mistralai/Mistral-Nemo-Base-2407',type=str, help="Tokenizer文件")
