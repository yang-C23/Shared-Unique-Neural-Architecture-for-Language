...
def main():
    ...
    model = create_hf_model(AutoModelForCausalLM,
                            ...
                            tokenizer,
                            ds_config=None, # ds_config might be used by deepspeed.initialize
    ...
    #第三个验证数据集
    # data_module2 = _make_supervised_data_module(args,train_data_prefix=args.pretrain_train_data_path2,eval_data_prefix=args.pretrain_test_data_path2,
    #                                            pad_id=tokenizer.eos_token_id)
    # french_eval_dataset = data_module2[\'eval_dataset\']

    
    train_dataset = data_module[\'train_dataset\'] # Needed for print_rank_0 below, but train_dataloader is not used.
    chinese_eval_dataset = data_module[\'eval_dataset\']
    train_data_collator = data_module[\'train_data_collator\'] # Needed for print_rank_0 below
    chinese_eval_data_collator = data_module[\'eval_data_collator\']
    
    # This section prints some debug info using train_dataset, keep for now
    ...
    # print_rank_0(\'len_train_sample:{}\'.format(len(train_dataset)), args.global_rank) # Related to training
    ...
    def evaluation(model, eval_dataloader):
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            loss = outputs.loss
            losses += loss.float()
            # if step + 1 >= 20: # Keep this for potentially faster eval if needed by user
            #     break
    ...
    # Commenting out optimizer and scheduler setup
    ...
    #     len(train_dataloader) / args.gradient_accumulation_steps) # train_dataloader is commented
    ...
    # model, optimizer, _, lr_scheduler = deepspeed.initialize( # optimizer and lr_scheduler are commented
    #     model=model,
    ...
    # Initialize model with DeepSpeed (no optimizer or scheduler for eval-only mode)
    model, *_ = deepspeed.initialize(
        model=model,
        args=args, # args still contains lr, weight_decay etc. but they won't be used by optimizer
        config=ds_config,
        dist_init_required=True
    )


    # if args.gradient_checkpointing: # Not training
    #     model.gradient_checkpointing_enable()

    # # Train! # Commenting out training-specific variables
    ...
    # #save_samples = [2000,5000, 10000, 20000, 40000]
    # #save_samples = [100,2000,5000]
    # save_samples = [0] # This was used for saving checkpoints during training
    # save_steps = [math.ceil(samples/batch_size) for samples in save_samples]
    # save_dict = dict(zip(save_steps,save_samples))
    # print_rank_0(
    #         "save msg:", # Related to training checkpoints
    #         args.global_rank)
    # print_rank_0(
    #         save_dict, # Related to training checkpoints
    #         args.global_rank)
    ...
    #####
        if args.output_dir is not None:
            print_rank_0(\'saving the perturbed model ...\', args.global_rank) # Changed message
            model = convert_lora_to_linear_layer(model) # This might be relevant if LoRA was used
...
########freeze region train 03/05/2025 -> evaluation of perturbed model
    print_rank_0(
            f"***** Evaluating perplexity on Chinese after perturbation *****", # Changed message
            args.global_rank)
    perplexity = evaluation(model, chinese_eval_dataloader)
    print_rank_0(f"ppl on Chinese post-perturbation: {perplexity}", args.global_rank) # Changed message

    print_rank_0(
            f"***** Evaluating perplexity on English after perturbation *****", # Changed message
            args.global_rank)
    perplexity = evaluation(model, english_eval_dataloader)
    print_rank_0(f"ppl on English post-perturbation: {perplexity}", args.global_rank) # Changed message

    if hasattr(torch.cuda, \'empty_cache\'):
        torch.cuda.empty_cache()

    
    # print_rank_0("***** Running training *****", args.global_rank) # Training loop commented out
    ...
    # # Evaluate perplexity on the validation set. # This block is after training, not needed.
...
########freeze region train 03/05/2025 -> evaluation of perturbed model
    ...
    # #     args.global_rank)
    # # perplexity = evaluation(model, eval_dataloader) # eval_dataloader was chinese_eval_dataloader
    ...
    # # if args.output_dir is not None:
    # #     print_rank_0(\'saving the final model ...\', args.global_rank) # Already saved perturbed model
    # #     model = convert_lora_to_linear_layer(model)
    ...
    # #                               args.output_dir,
    # #                               zero_stage=args.zero_stage)
    print_rank_0("Evaluation of perturbed model complete.", args.global_rank)
