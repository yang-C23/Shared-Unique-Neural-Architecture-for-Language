...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Perturb and save a Mistral-NeMo-Minitron-8B-Base model based on region masks.")
    parser.add_argument(
        "--model_name_or_path",
        type=str,
    ...
    parser.add_argument("--cpu_offload_masks",
                        action='store_true', 
                        help="Keep masks on CPU to reduce GPU memory usage.")
    parser.add_argument("--perturbation_type",
                        type=str,
                        choices=['zero_out', 'preserve_frozen'],
                        default='zero_out',
                        help="Type of perturbation: 'zero_out' zeros masked regions, 'preserve_frozen' preserves them.")
    parser.add_argument("--force_safetensors",
                        action='store_true',
                        default=True,
                        help="Force saving in safetensors format to avoid torch.load security issues.")
...
def main():
    ...
    # Load tokenizer - use AutoTokenizer for Mistral models
    try:
        tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path)
    except:
        # Fallback to load_hf_tokenizer if available
    ...
    print_rank_0(f"Loading Mistral model from {args.model_name_or_path}...", args.global_rank)
    
    try:
        # Try using create_hf_model if available
        ...
        # Fallback to direct AutoModelForCausalLM loading
        print_rank_0("Fallback to direct model loading...", args.global_rank)
    ...
    with torch.no_grad():
        ...
        for name, param in model_to_pertub.named_parameters():
            # Adapt parameter filtering for Mistral model structure
            if 'layers.' not in name or 'norm' in name or 'embed' in name:
                continue
            ...
            if args.random_ratio > 0:
                try:
                    ...
                    if args.cpu_offload_masks:
                        weight_to_freeze = weight_to_freeze.cpu()
                    
                    # Log generation info
                    print_rank_0(f"Generated random mask for {name} - Frozen: {weight_to_freeze.sum().item()}", args.global_rank)
                    
                    # Apply perturbation logic directly here for random case
                    # Apply perturbation based on type
                    if args.perturbation_type == 'zero_out':
                    ...
                    elif args.perturbation_type == 'preserve_frozen':
                        # Preserve frozen regions, zero out non-frozen regions
                        if args.cpu_offload_masks:
                            param_cpu = param.data.cpu()
                            # Zero out non-frozen regions
                            param_cpu[~weight_to_freeze] = 0
                            param.data.copy_(param_cpu)
                            del param_cpu
                        else:
                    ...
                    print_rank_0(f"Applied random {args.perturbation_type} perturbation to {name}", args.global_rank)
            ...
            try:
                ...
                print_rank_0(f"Applied {args.perturbation_type} perturbation to {name}", args.global_rank)
    ...
    if args.output_dir is not None:
        print_rank_0(f'Saving perturbed Mistral model to {args.output_dir}...', args.global_rank)
        ...
        if args.global_rank == 0:
             ...
             # CRITICAL FIX: Force safetensors format to avoid torch.load security issues
             if args.force_safetensors:
                 print_rank_0("Saving model with safetensors format to avoid torch.load security issues...", args.global_rank)
             else:
                 print_rank_0("Saving model in standard format...", args.global_rank)
             
             # CRITICAL FIX: Always use direct safetensors save when requested
             if args.force_safetensors:
                 print_rank_0("Using DIRECT safetensors save to ensure proper format...", args.global_rank)
                 # Skip save_hf_format and use direct safetensors save
                 model_to_save.save_pretrained(
                     args.output_dir,
                     safe_serialization=True,  # Force safetensors format
                     max_shard_size="5GB"      # Prevent memory issues with large models
                 )
                 tokenizer.save_pretrained(args.output_dir)
                 print_rank_0("Direct safetensors save completed.", args.global_rank)
             else:
                 # Try using save_hf_format for legacy compatibility
                 try:
                     save_hf_format(model_to_save, tokenizer, args)
                 except:
                     print_rank_0("Using standard Hugging Face save method...", args.global_rank)
        ...
        if args.zero_stage == 3:
            ...
            if args.force_safetensors:
                print_rank_0("Using ZeRO-3 save function with safetensors format...", args.global_rank)
            else:
                print_rank_0("Using ZeRO-3 save function...", args.global_rank)
            
            # CRITICAL FIX: For ZeRO-3, always fallback to direct safetensors save if force_safetensors is True
            if args.force_safetensors:
                print_rank_0("ZeRO-3 with safetensors: Using direct save method to ensure safetensors format...", args.global_rank)
                # For ZeRO-3, we need to use the consolidated save approach
                # Skip save_zero_three_model and use direct save with consolidation
                if args.global_rank == 0:
                    model_to_save.save_pretrained(
                        args.output_dir,
                ...
                print_rank_0("ZeRO-3 direct safetensors save completed.", args.global_rank)
            else:
                # Try standard ZeRO-3 save
                try:
                    ...
                    print_rank_0(f"ZeRO-3 save failed: {e}. Falling back to standard save...", args.global_rank)
                    if args.global_rank == 0:
                        model_to_save.save_pretrained(args.output_dir)
        ...
        print_rank_0("Mistral model saving complete.", args.global_rank)
    # --- End Saving Logic ---
