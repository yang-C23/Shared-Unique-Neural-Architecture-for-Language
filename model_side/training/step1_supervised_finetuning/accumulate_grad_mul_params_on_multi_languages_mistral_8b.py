...
# Set comprehensive offline mode for transformers to prevent internet access
os.environ["TRANSFORMERS_OFFLINE"] = "1"
os.environ["HF_DATASETS_OFFLINE"] = "1"
os.environ["TRANSFORMERS_CACHE"] = "/leonardo_work/EUHPC_B24_036/yang/.cache/huggingface"
os.environ["HF_HOME"] = "/leonardo_work/EUHPC_B24_036/yang/.cache/huggingface"
os.environ["HF_HUB_OFFLINE"] = "1"
os.environ["HUGGINGFACE_HUB_CACHE"] = "/leonardo_work/EUHPC_B24_036/yang/.cache/huggingface"

# Disable network access for safety
import requests.adapters
import urllib3.poolmanager
original_urlopen = urllib3.poolmanager.PoolManager.urlopen
def disabled_urlopen(self, method, url, *args, **kwargs):
    raise Exception(f"Network access disabled! Attempted to access: {url}")
urllib3.poolmanager.PoolManager.urlopen = disabled_urlopen

from tqdm import tqdm
import torch
from torch.utils.data import DataLoader, RandomSampler, SequentialSampler
from torch.utils.data.distributed import DistributedSampler
# Remove the problematic CUDA memory configuration that causes fragmentation
...
class MyDataset(Dataset):
    def __init__(self, data_prefix, seq_length, pad_id):
        ...
        """比如：/mistral/our/data"""
        """后面会根据需要自动的添加上/mistral/our/data.idx"""
        """后面会根据需要自动的添加上/mistral/our/data.bin"""
        """后面会根据需要自动的添加上/mistral/our/data.dis"""
    ...
    def _load_index(self):
        ...
        """样本总数 - Updated for Mistral uint32 length storage"""
    ...
    def _load_bin(self):
        """以内存映射的方式进行加载大文件 - Updated for Mistral uint32 token storage"""
...
def main():
    ...
    print_rank_0(f"Initial CPU memory usage: {torch.cuda.memory_allocated() / 1024**3:.2f} GB" if torch.cuda.is_available() else "CUDA not available", 0)
    ...
    # Print memory usage after distributed initialization
    print_rank_0(f"Memory after distributed init: {torch.cuda.memory_allocated() / 1024**3:.2f} GB" if torch.cuda.is_available() else "", args.global_rank)
    ...
    # Mistral-specific tokenizer configuration
    # Mistral uses different token IDs: BOS=1, EOS=2
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token
    # make sure tokenizer is right pad in our logic
    tokenizer.padding_side = 'right'

    # CRITICAL FIX: Pass ds_config to enable ZeRO Stage 3 model partitioning
    # This prevents loading the entire model into memory on each process
    model = create_hf_model(AutoModelForCausalLM,
                            args.model_name_or_path,
                            tokenizer,
                            # ds_config=None,
                            ds_config=ds_config,  # Enable ZeRO Stage 3 initialization
                            disable_dropout=args.disable_dropout)
    
    # Print memory usage after model creation
    print_rank_0(f"Memory after model creation: {torch.cuda.memory_allocated() / 1024**3:.2f} GB" if torch.cuda.is_available() else "", args.global_rank)
    
    # Clear memory after model creation
    if hasattr(torch.cuda, 'empty_cache'):
        torch.cuda.empty_cache()
    
    # Use proper pad_token_id for Mistral (which is same as eos_token_id for Mistral)
    pad_token_id = tokenizer.pad_token_id if tokenizer.pad_token_id is not None else tokenizer.eos_token_id
    
    # Defer dataset loading until after model partitioning to reduce memory pressure
    ...
    # Print memory usage after dataset loading
    print_rank_0(f"Memory after dataset loading: {torch.cuda.memory_allocated() / 1024**3:.2f} GB" if torch.cuda.is_available() else "", args.global_rank)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader), unit="batch"):
            ...
            # model.optimizer.overlap_grad_reduce_wait()
            # AttributeError: 'DeepSpeedZeroOptimizer_Stage3' object has no attribute 'overlap_grad_reduce_wait'
            # model.optimizer.sync_all_params_views()
            ...
            if (step+1) in save_steps:
                ...
                for n, lp in model.named_parameters():
                    ...
                    # stage 3 需要使用full fp32 parameter
                    ...
                    if args.global_rank == 0:
                        ...
                        # Use full parameter (hp) instead of sharded parameter (lp) to match gradient dimensions
                        #grad_mul_param_tensor = torch.mul(hp_grad, hp)
                        #torch.save(grad_mul_param_tensor.bfloat16(), save_path)

                        if hp_grad is not None and hp is not None:
                            grad_mul_param_tensor = torch.mul(hp_grad, hp)
                            torch.save(grad_mul_param_tensor.bfloat16(), save_path)
                        else:
                            print_rank_0(f"Warning: {n} has no gradient or parameter", args.global_rank)

                # Ensure all processes wait for rank 0 to finish saving before proceeding.
                torch.distributed.barrier()
