...
# Import from our standalone utils
...
class MyDataset(Dataset):
    def __init__(self, data_prefix, seq_length, pad_id):
        ...
        """比如：/qwen/our/data"""
        """后面会根据需要自动的添加上/qwen/our/data.idx"""
        """后面会根据需要自动的添加上/qwen/our/data.bin"""
        """后面会根据需要自动的添加上/qwen/our/data.dis"""
    ...
    def _load_index(self):
        ...
        """样本总数 - Updated for Qwen uint32 length storage"""
    ...
    def _load_bin(self):
        """以内存映射的方式进行加载大文件 - Updated for Qwen uint32 token storage"""
...
def main():
    ...
    # Qwen-specific tokenizer configuration
    ...
    # Use pad_token_id for Qwen (which is same as eos_token_id for Qwen)
